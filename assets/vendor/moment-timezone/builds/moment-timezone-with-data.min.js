! function(M, b) {
    "use strict";
    "object" == typeof module && module.exports ? module.exports = b(require("moment")) : "function" == typeof define && define.amd ? define(["moment"], b) : b(M.moment)
}(this, function(A) {
    "use strict";
    ...
    var b, c = {},
        ...
        W = {},
        d = {};
    A && "string" == typeof A.version || m("Moment Timezone requires Moment.js. See https://momentjs.com/timezone/docs/#/use-it/browser/");
    ...
    function O(M) {
        var b = 0,
            ...
            c = 0,
            q = 1;
        for (45 === M.charCodeAt(0) && (q = -(b = 1)); b < p.length; b++) c = 60 * c + n(p.charCodeAt(b));
    ...
    function N(M, b) {
        for (var z, p; p = 6e4 * ((b.at - M.at) / 12e4 | 0);)(z = new f(new Date(M.at + p))).offset === M.offset ? M = z : b = z;
        return M
    }
    ...
    function u(M, b) {
        var z, p;
        for (R(b), z = 0; z < b.length; z++) p = b[z], d[p] = d[p] || {}, d[p][M] = !0
    }
    ...
    function e() {
        try {
            ...
            if (M && 3 < M.length) {
                var b = W[r(M)];
                if (b) return b;
                m("Moment Timezone found " + M + " from the Intl api, but did not have that data loaded.")
            }
        } catch (M) {}
        var z, p, O, A = function() {
                var M, b, z, p = (new Date).getFullYear() - 2,
                    O = new f(new Date(p, 0, 1)),
                    A = [O];
                for (z = 1; z < 48; z++)(b = new f(new Date(p, z, 1))).offset !== O.offset && (M = N(O, b), A.push(M), A.push(new f(new Date(M.at + 6e4)))), O = b;
                for (z = 0; z < 4; z++) A.push(new f(new Date(p + z, 0, 1))), A.push(new f(new Date(p + z, 6, 1)));
                return A
            }(),
            c = A.length,
            q = function(M) {
                var b, z, p, O = M.length,
                    ...
                    c = [];
                for (b = 0; b < O; b++)
                    for (z in p = d[M[b].offset] || {}) p.hasOwnProperty(z) && (A[z] = !0);
    ...
    function T(M) {
        var b, z, p, O;
        for ("string" == typeof M && (M = [M]), b = 0; b < M.length; b++) O = r(z = (p = M[b].split("|"))[0]), c[O] = M[b], W[O] = z, u(O, p[2].split(" "))
    }

    function t(M, b) {
        M = r(M);
        var z, p = c[M];
        return p instanceof B ? p : "string" == typeof p ? (p = new B(p), c[M] = p) : q[M] && b !== t && (z = t(q[M], t)) ? ((p = c[M] = new B)._set(z), p.name = W[M], p) : null
    }

    function s(M) {
        var b, z, p, O;
        for ("string" == typeof M && (M = [M]), b = 0; b < M.length; b++) p = r((z = M[b].split("|"))[0]), O = r(z[1]), q[p] = O, W[p] = z[0], q[O] = p, W[O] = z[1]
    }
    ...
    function C(M) {
        T(M.zones), s(M.links),
            function(M) {
                var b, z, p, O;
                if (M && M.length)
                    for (b = 0; b < M.length; b++) z = (O = M[b].split("|"))[0].toUpperCase(), p = O[1].split(" "), o[z] = new L(z, p)
            }(M.countries), S.dataVersion = M.version
    }
    ...
    function S(M) {
        ...
        return p && !A.isMoment(M) && E(O) && O.add(p.parse(O), "minutes"), O.tz(z), O
    }(z < 2 || 2 == z && p < 6) && m("Moment Timezone requires Moment.js >= 2.6.0. You are using Moment.js " + A.version + ". See momentjs.com"), B.prototype = {
        _set: function(M) {
        ...
        parse: function(M) {
            var b, z, p, O, A = +M,
                ...
                o = q.length - 1;
            for (O = 0; O < o; O++)
                if (b = c[O], z = c[O + 1], p = c[O ? O - 1 : O], b < z && S.moveAmbiguousForward ? b = z : p < b && S.moveInvalidForward && (b = p), A < q[O] - 6e4 * b) return c[O];
            return c[o]
        },
        abbr: function(M) {
        ...
        offset: function(M) {
            return m("zone.offset has been deprecated in favor of zone.utcOffset"), this.offsets[this._index(M)]
        },
    ...
    }, S.version = "0.5.36", S.dataVersion = "", S._zones = c, S._links = q, S._names = W, S._countries = o, S.add = T, S.link = s, S.load = C, S.zone = t, S.zoneExists = function M(b) {
        return M.didShowError || (M.didShowError = !0, m("moment.tz.zoneExists('" + b + "') has been deprecated in favor of !moment.tz.zone('" + b + "')")), !!t(b)
    }, S.guess = function(M) {
        return b && !M || (b = e()), b
    }, S.names = function() {
        var M, b = [];
        for (M in W) W.hasOwnProperty(M) && (c[M] || c[q[M]]) && W[M] && b.push(W[M]);
        return b.sort()
    }, S.Zone = B, S.unpack = a, S.unpackBase60 = O, S.needsOffset = E, S.moveInvalidForward = !0, S.moveAmbiguousForward = !1, S.countries = function() {
        ...
        var z = M.zones.sort();
        return b ? z.map(function(M) {
            return {
                name: M,
                offset: t(M).utcOffset(new Date)
            }
        }) : z
    };
    var g, P = A.fn;
    ...
    A.tz = S, A.defaultZone = null, A.updateOffset = function(M, b) {
        var z, p = A.defaultZone;
        if (void 0 === M._z && (p && E(M) && !M._isUTC && (M._d = A.utc(M._a)._d, M.utc().add(p.parse(M), "minutes")), M._z = p), M._z)
            if (z = M._z.utcOffset(M), Math.abs(z) < 16 && (z /= 60), void 0 !== M.utcOffset) {
                var O = M._z;
        ...
        if (M) {
            ...
            return this._z = t(M), this._z ? A.updateOffset(this, b) : m("Moment Timezone has no data for " + M + ". See http://momentjs.com/timezone/docs/#/data-loading/."), this
        }
